...
This exam includes four questions that test different aspects of your learning during this course: data wrangling, data visualisation and statistics. Each question is broken down into a number of sub-questions. 
...
`TEMPMAX` is the maximum temperature of the day (Celsius).
...
**2) Character displacement in lizards (10 points)**


Character displacement is the phenomenon where differences among similar species with overlapping geographic distributions are accentuated in regions where the species co-occur. This means that a competitor's presence can influence the morphology of a species (e.g., adult body size).
...
These data are stored in the file `crabFights.csv`. The data set has seven columns. The first two columns are the ID numbers of the focal and opponent crabs. Columns 3 and 4 give the carapace width (`focal_carapaceWidth`) and chela length (`focal_chelaLength`) of the focal crab in millimeters. Columns 5 and 6 provide the same information for the opponent crab. The final column (`fightOutcome`), gives the outcome of the fight: `won` indicates that the focal crab won the fight, `lost` means that it lost the fight.
