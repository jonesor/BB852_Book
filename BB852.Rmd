...
```{r echo = FALSE}
curl::curl_download("https://mitsdu.sdu.dk/skema/icalendar/activities/N110040101/e20/e20",destfile = "bb852.ics")
...
library(calendar)
```
## Exercise Solutions: Californian bird diversity
...
<!--chapter:end:0_01.1-ExerciseSolutions.Rmd-->
...
<!--chapter:end:1_01.2-amnioteSolutions.Rmd-->
...
<!--chapter:end:1_02.1-eggLayingDateSolutionsRmd.Rmd-->
...
rn <- pivot_longer(rn,cols= c(normDist,poisDist), names_to = "sampleID", values_to = "value") #rearrange data
...
```{r echo=-1, fig.cap=('Barplot of 6 simulated coin tosses')}
set.seed(2)
...
```{r echo=-1,fig.cap=('Barplot of 90 simulated dice throws')}
set.seed(2)
n <- 90
...
2. Try answering the question - "*are people who prefer dogs taller than those who prefer cats?* " using the `courseData.csv`. Can you think of any problems with this analysis?
```
...
```{block, type="do-something"}
1. Try refitting the model using simply the proportion female (`propFemale`) data rather than the two-column (`cbind`) approach.
2. Try writing up the result in the same way as shown for the NFL field goals example.
```
