...
Most of you already know, from previous courses, how to use R to make simple plots (e.g. `plot(x,y)` and `hist(x)`). In this course we introduce a new approach using the R package `ggplot2`. The `gg` part in the package name stands for _Grammar of Graphics_, a framework for thinking about visualisations developed by Leland Wilkinson in his book of the same name. The basic idea behind the grammar of graphics is that any visualization can be thought of as a combination of building blocks, like the grammar in a sentence. These building blocks include scales, geoms (or geometric objects), and aesthetic mappings, which control the appearance of the visualization.

By breaking down visualizations into these constituent parts, the grammar of graphics provides a way to systematically create and compare different types of visualisations for the same data. This approach also makes it possible to create complex visualizations by combining simple building blocks, making it easier to understand and reuse code.

The grammar of graphics has been implemented in several data visualization software packages, including `ggplot2` in R. In this chapter you will be guided through using the `ggplot2` package to make some effective plots. You will therefore need the `ggplot2` package to make this work. Remember, you can load packages like this:
