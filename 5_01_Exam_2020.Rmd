...
amazon <- read.csv("CourseData/amazon.csv") %>%
  mutate(number = ifelse(number >= 1000, 1 + (number - 1000) / 1000, number))
...
# Check that all are included in the result (you might need to check and fix "foreign" letters - a typical data cleaning task!)
legalAmazonStates <- c("Acre", "Amapa", "Pará", "Amazonas", "Rondonia", "Roraima", "Mato Grosso", "Tocantins", "Maranhao")

legalAmazon <- amazon %>%
  filter(state %in% legalAmazonStates)


# Next make a "look up" table to convert month name to a numeric value 1-12
(monthLookup <- amazon %>%
  select(month) %>%
  unique() %>%
...
# then group_by month and year, and summarise to get total number (i.e. sum for each month in each year for whole amazon)
...
table1 <- legalAmazon %>%
  ...
  summarise(minimum = min(sumFires), mean = mean(sumFires), median = median(sumFires), maximum = max(sumFires))
...
table1 <- legalAmazon %>%
  left_join(monthLookup) %>%
  group_by(month_numeric) %>%
  summarise(minimum = min(number), mean = mean(number), median = median(number), maximum = max(number))
...
```{r}
# Use summarise to calculate the the total number of fires per month
# In this case I use the legal amazon, but it is acceptable to look at the whole
# data set. You need to make sure that you are using the numeric month.

# Group by year and month to calculate the total number of fires in each year and month.

# Create the dataset by filtering to legal amazon, adding numeric month, grouping by year and month and finally summarising.
amazon2 <- amazon %>%
  filter(state %in% legalAmazonStates) %>%
  left_join(monthLookup) %>%
...
```{r}
oilseed <- read.csv("CourseData/oilseed.csv", stringsAsFactors = TRUE)
...
```{r}
roundabouts <- read.csv("CourseData/roundabouts.csv", stringsAsFactors = TRUE)

ggplot(roundabouts, aes(x = area, y = nSpecies, colour = habitatType)) +
  geom_point()
```
...
mod1 <- glm(nSpecies ~ area * habitatType, family = "poisson", data = roundabouts)
autoplot(mod1)
anova(mod1, test = "Chi")
summary(mod1)
```
...
newData <- expand.grid(area = seq(0, 50, 0.1), habitatType = c("complex", "simple"))
