...
currentFilesNotUsed <- currentFiles[which(!currentFiles %in% dataSetsUsed$name)]

if (length(currentFilesNotUsed) > 0) {
  delFiles <- paste0("CourseData/", currentFilesNotUsed)
...
if(nrow(dataSetsMissing)>0){stop("One or more datasets mentioned in the text are missing. Check dataSetsMissing.csv.")}
