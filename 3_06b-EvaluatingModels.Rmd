...
The Akaike Information Criterion (AIC) is a statistic used to compare the relative quality of models for a given data set: it helps you decide which of a set of candidate models is the best.  The statistic was developed in 1970 by a Japanese statistician called Hirotugu Akaike. AIC values are only comparable among models that use the same data, and which have the same response variable (thus they need the same error structure). AIC balances the trade-off between a model's ability to fit the data well (i.e., have a low residual sum of squares) and the number of parameters in the model. Lower AIC values indicate a better model fit, as they suggest that the model has a lower risk of overfitting the data, while still fitting the data well. In this context, "overfitting" refers to a model that fits the data too well, to the point that it captures the noise/random fluctuations in the data rather than the underlying patterns. Essentially, an overfit model is too complex (too many parameters) and not very well generalisable to new data. In contrast, a well-fit model has a good balance between fitting the data well and being simple enough to generalize well to new data.

If you are interested in the mysterious details of what this quantity is you can read further on Wikipedia, or a more advanced statistics book, otherwise you can simply trust that AIC estimates the relative quality of model, with lower values being better.
