...
courseCal <- ic_read("bb852.ics") %>% 
  ...
  mutate(Date = paste0( month(DTSTART2,label = TRUE)," ",day(DTSTART2),", ",wd,". kl.", St,"-",En)) %>% 
  select(Session,Date, Room)
...
schedule2 <- schedule %>% 
  select(Part, Date, Room, Session, Type, Topic, GSWR, CourseBook = `Course Book`)

big_b <- fp_border(color="gray70", width = 3)
std_b <- fp_border(color="gray70", style = "dashed")
big_b_end <- fp_border(color="white", width = 3)

ft <- flextable(schedule2, cwidth = c(2,6,1,.5,1,4,.75,.75)) %>% 
  bold(part = "header") %>% 
  bold(j = 1) %>% 
