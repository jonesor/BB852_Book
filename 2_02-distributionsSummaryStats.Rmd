...
- Are larger ponds home to more amphibian species?  
...
These variables are also known as **dependent** (response) and **independent** (explanatory) variables. However, in biology, we prefer the terms "response" and "explanatory" because these terms are more neutral about causation. For example, an increase in temperature might correlate with earlier flowering times in plants, but this relationship could also be influenced by other factors, like changes in soil moisture or pollinator activity rather than being a direct causal relationship.

Think of it like this: the response variable is the outcome, and the explanatory variable is what you suspect drives that outcome. These roles are crucial to keep straight, not only when you analyse your data but also when you visualise it. For instance, by convention, the response variable is plotted on the y-axis and the explanatory variable on the x-axis.

## Populations, samples and bias

In our studies, we often want to make generalisations about a **population**—the entire group of individuals, sites, or events we’re interested in. For instance, you might study a population of birds in a forest or all individual trees of a particular species in a forest. Since it’s rarely feasible to measure every individual in a population, we collect a **sample**, a smaller subset.

A good sample is:

- **Representative**: It captures the variability present in the population.
- **Random**: Every individual or site has an equal chance of being included, reducing **selection bias**.

#### Example: Avoiding Sampling Bias in Bird Surveys
If you only survey birds near hiking trails, you might overestimate the abundance of species that prefer disturbed habitats, like robins or jays. Randomly selecting survey locations across the entire area ensures your data reflect the true bird community.
